...
import { WeavrnClient, type WeavrnEvent } from "@weavrn/sdk";
...
// ──────────────────────────────────────────────────────────────────────────
// Client abstraction — the production implementation uses @weavrn/sdk:
// `WeavrnClient` drives createJob/createEscrow*/linkJobEscrow/approveJob,
// and its SSE `subscribe(cb)` replaces the prior 3-second polling stub.
// Tests inject a stub implementing this interface; the shape is kept narrow
// so the stub surface stays small and the SDK's internals are not re-tested
// at the page level.
// ──────────────────────────────────────────────────────────────────────────
...
function strategyAddressFor(
  client: WeavrnClient,
  strategyName: string,
  override: string | null | undefined,
): string {
  if (override) return override;
  const map = client.strategies;
  if (strategyName === "milestone") return map.milestone;
  if (strategyName === "trickle") return map.trickle;
  return map.allOrNothing;
}

function mapWeavrnEventJobId(event: WeavrnEvent): number | null {
  const data = (event.data ?? {}) as { jobId?: unknown; job_id?: unknown };
  const raw = data.jobId ?? data.job_id;
  const n = typeof raw === "number" ? raw : Number(raw);
  return Number.isFinite(n) && n > 0 ? n : null;
}

export function buildLiveClient(deps: LiveClientDeps): ToolDetailClient {
  const { signer } = deps;
  const chainId = resolveChainId();
  const client = new WeavrnClient({ signer, chainId });

  return {
    async createJob(args) {
      const job = await client.createJob(args);
      return { id: job.id };
    },
    async createEscrowETH(args) {
      const price = ethers.parseEther(args.amountEth);
      const deadline = Math.floor(Date.now() / 1000) + args.deadlineSeconds;
      const strategyAddr = strategyAddressFor(
        client,
        args.strategyName,
        args.strategyAddress ?? null,
      );
      const res = await client.createEscrowETH(
        args.recipient,
        price,
        deadline,
        strategyAddr,
      ...
      return { escrowId: res.escrowId };
    },
    async createEscrowERC20(args) {
      // SDK-backed flow supports ERC-20: approve the escrow contract, then
      // create the escrow via the existing client helper. Keeps the page
      // honest when/if publish wizards start emitting ERC-20-priced tools.
      ...
      await client.approveEscrow(args.token, amount);
      const res = await client.createEscrowERC20(
        args.recipient,
        args.token,
        amount,
        deadline,
    ...
    async linkJobEscrow(args) {
      // WeavrnClient.linkJobEscrow has its own indexer-lag retry loop.
      await client.linkJobEscrow(args.jobId, args.escrowId);
    },
    async getJob(jobId) {
      // Keep the landing's session-based reader for the delivered-job fetch.
      // Users who reach this page went through the wallet sign-in flow, so
      // the Bearer path is readily available; no signed-GET roundtrip is
      // needed just to render the output.
      return apiGetJob(jobId);
    },
    async approveJob(args) {
      // WeavrnClient.approveJob releases escrow (no-op if already released)
      // and then calls completeJob with indexer-lag retry.
      await client.approveJob(args.jobId, args.escrowId);
    },
    subscribe(cb) {
      ...
      client
        .subscribe(async (event: WeavrnEvent) => {
          if (cancelled) return;
          const jobId = mapWeavrnEventJobId(event);
          ...
          if (event.type === "job_delivered") {
            ...
            try {
              ...
              // If the session fetch fails the page still exits the waiting
              // state cleanly via the absence of job.deliverable_data; users
              // can retry the release flow.
            }
            if (!cancelled) cb({ type: "job_delivered", jobId, job });
          } else if (event.type === "job_failed") {
            const data = (event.data ?? {}) as { error?: unknown };
            const errMsg =
              typeof data.error === "string" ? data.error : undefined;
            if (!cancelled) cb({ type: "job_failed", jobId, error: errMsg });
          } else if (event.type === "job_disputed") {
            if (!cancelled) cb({ type: "job_disputed", jobId });
          }
        })
        .then((fn) => {
          if (cancelled) {
        ...
        .catch(() => {
          // SDK subscribe() rejected (auth, bad signer, transient). The
          // page's timeout will still fire; swallow here so the caller
          // doesn't see a dangling rejection.
        });
...
export function ToolDetailView(props: ToolDetailViewProps): JSX.Element {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const priceTokenRaw = (tool.price_token ?? "").trim();
    const usesEth = priceTokenRaw === "" || priceTokenRaw.toUpperCase() === "ETH";
    const usesErc20 = /^0x[a-fA-F0-9]{40}$/.test(priceTokenRaw);
    if (!usesEth && !usesErc20) {
      setErrorText(
        `Unsupported price token: ${tool.price_token}. Expected "ETH" or an ERC-20 contract address.`,
      );
      return;
    }
    ...
    try {
      ...
      const escrow = usesEth
        ? await client.createEscrowETH({
        ...
        : await client.createEscrowERC20({
            recipient: tool.wallet_address,
            token: priceTokenRaw,
