...
import { getAgentPayments, getAgentEscrows } from "@/lib/api";
import type { PaymentRecord, EscrowRecord } from "@/lib/api";
...
export default function AgentDashboard({ walletAddress, signer }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [payRes, escRes] = await Promise.all([
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      {/* Primary: Agent management with nested listings */}
      <AgentSetup walletAddress={walletAddress} agentName="" signer={signer} />

      {/* Jobs across all agents */}
      <JobQueue
